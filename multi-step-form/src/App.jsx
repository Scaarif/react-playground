...
function App() {
  ...
  return (
   <div className="flex justify-center items-center md:p-8 gap-8 md:pt-10 md:pb-4 font-display max-w-[1440px] min-h-screen bg-Magnolia">
    <div className="flex flex-col md:flex-row md:w-[850px] w-full md:min-h-[500px] h-full gap-2 md:p-4 md:bg-White rounded-none md:rounded-lg">
      {/* Sidebar */}
      <div className="flex flex-row justify-center items-center md:justify-start md:items-start md:flex-col
        gap-6 py-12 pb-16 md:pb-8 md:p-8 md:pr-20 bg-sidebarMobile md:bg-sidebarDesktop bg-cover md:rounded-md md:min-w-[250px]">
        {/* item */}
      ...
      {/* Form */}
      {!submitted ?
        <form onSubmit={(e) => {e.preventDefault(); setSubmitted(true)}} className="m-4 -mt-16 md:m-0 bg-White md:bg-transparent rounded-md md:rounded-0 flex-1 flex flex-col gap-8 p-8 md:pl-24 md:pr-16 md:min-h-[500px]">
        {/* header */}
        <div className="flex flex-col gap-2">
          <span className="text-3xl font-bold text-MarineBlue">{items[Object.keys(items)[active]][0]}</span>
          ...
          {personalInfo.map((item, i) =>
            <div key={item.id} className="flex flex-col gap-1">
              <div className="flex justify-between items-center">
                <label htmlFor={item.name} className="capitalize text-MarineBlue font-semibold text-sm">{item.label}</label>
                <span className={`${error[i] === 0 ? 'flex' : 'hidden'} text-StrawberryRed font-semibold text-sm`}>This field is required</span>
              </div>
              <input
                type="text"
                name={item.name}
                ...
                required
                className={`border ${error[i] === 0 ? 'border-StrawberryRed' : 'border-LightGray'} rounded-md p-4 py-3 text-MarineBlue font-semibold text-sm
                  focus:outline-none focus:border-PurplishBlue cursor-pointer`}
              />
            </div>
            )}
        </div>
        ...
        <div className="flex flex-col gap-8">
          <div className="flex gap-4">
            ...
            {['arcade', 'advanced', 'pro'].map(plan => 
              <div key={plan} className={`flex flex-col gap-8 border rounded-md p-3 pr-8 ${user.plan.name === plan ? 'border-PurplishBlue bg-Magnolia' : 'border-LightGray'}
                ...
                <div className="flex flex-col gap-1">
                  <span className="capitalize text-MarineBlue font-semibold">{plan}</span>
                  <span className="text-CoolGray text-sm">{user.plan.subscription === 'monthly' ? `$${plans[plan] / 10}/mo` : `$${plans[plan]}/yr`}</span>
                  {user.plan.subscription === 'yearly' && <span className="text-xs text-MarineBlue font-medium">2 months free</span>}
                </div>
              </div>
              )}
          </div>
          <div className="flex gap-4 justify-center items-center p-3 bg-Magnolia rounded-md text-sm font-semibold">
            ...
            <span className="bg-MarineBlue flex items-center justify-between w-8 p-1 rounded-xl">
              <span
                className={`${user.plan?.subscription === 'monthly' ? 'bg-White' : 'bg-transparent'} transition-colors h-3 w-3 rounded-full`}
                onClick={() => setUser({ ...user, plan: {...user.plan, subscription: 'monthly'}})}
              ></span>
              <span
                className={`${user.plan?.subscription === 'yearly' ? 'bg-White' : 'bg-transparent'} transition-colors h-3 w-3 rounded-full`}
        ...
        <div className="flex flex-col gap-4">
          {/* item */}
          { addOns.map((addOn, i) =>
            <div key={i} className={`flex items-center justify-between border rounded-md ${user.addons.includes(addOn.head) ? 'bg-Magnolia border-PurplishBlue' : 'border-LightGray'} p-4`}>
              <div className="flex items-center gap-4">
                <input type="checkbox" checked={user.addons.includes(addOn.head)}
                  className="bg-PurplishBlue w-5 h-5" onChange={() => handleCheck(addOn.head)} />
                <div className="flex flex-col gap-.5">
                  <span className="text-MarineBlue font-semibold">{addOn.head}</span>
                  <span className="text-sm text-CoolGray">{addOn.liner}</span>
                </div>
              </div>
              <span className="text-PurplishBlue text-sm font-medium">+${user.plan.subscription === 'monthly' ? `${addOn.cost}/mo` : `${addOn.cost * 10}/yr`}</span>
            </div>
          )
          }
        </div>
        ...
        <div className="flex flex-col gap-4">
          <div className="flex flex-col divide-y gap-4 bg-Magnolia rounded-md p-4">
            <div className="flex justify-between items-center text-md">
              <div className="flex flex-col gap-1">
                ...
                <span className="text-sm text-CoolGray underline cursor-pointer transition-colors hover:text-PurplishBlue"
                  onClick={(e) =>{e.stopPropagation(); setActive(1)}}
                >Change</span>
              </div>
              <span className="text-MarineBlue font-semibold">${user.plan.name ? (user.plan.subscription === 'monthly' ? `${plans[user.plan.name] / 10}/mo` : `${plans[user.plan.name]}/yr`) : 0}</span>
            </div>
          ...
          <div className="flex justify-between items-center">
            <span className="text-sm text-CoolGray">Total ({user.plan.subscription === 'monthly' ? 'per month' : 'per year'})</span>
            <span className="text-lg text-PurplishBlue font-semibold">
      ...
      :
        <div className="flex flex-col items-center justify-center gap-8 p-16 md:min-h-[500px]">
          <span className=""><img src={thanks} alt="" className="" /></span>
          <div className="flex flex-col items-center justify-center text-center gap-4">
            <span className="text-MarineBlue font-bold text-3xl">Thank you!</span>
            <span className="text-CoolGray">Thanks for confirming your subscription! We hope you have fun using our platform. If you ever need support, please feel free to email us at support@loremgaming.com</span>
          </div>
        </div>
      }
    </div>
